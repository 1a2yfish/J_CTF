-- =============================================
-- CTF竞赛系统数据库初始化脚本（合并用户表+统一名称字段版）
-- 数据库：competition_system
-- 适配：MySQL 8.0+（支持窗口函数、检查约束等）
-- 核心修改：1. 三表合并为单User表 2. 统一AdminName和userName为UserName字段
-- =============================================
...
-- 2. 统一用户表（User）：合并管理员/普通用户，统一名称为UserName
CREATE TABLE IF NOT EXISTS `User` (
                                      ...
                                      `UserType` VARCHAR(20) NOT NULL COMMENT '用户类型（ADMIN-管理员/ORDINARY-普通用户）',
                                      `UserName` VARCHAR(50) UNIQUE NOT NULL COMMENT '统一用户名/管理员姓名（唯一）',
    -- 管理员专属字段（仅UserType=ADMIN时有效）
                                      `AdminRole` VARCHAR(20) DEFAULT 'SYSTEM' COMMENT '管理员角色（SYSTEM/OPERATOR/AUDITOR）',
    -- 普通用户专属字段（仅UserType=ORDINARY时有效）
    ...
    -- 主键与约束
                                      PRIMARY KEY (`userID`),
    -- 检查约束：限制用户类型只能是ADMIN或ORDINARY
                                      CHECK (`UserType` IN ('ADMIN', 'ORDINARY')),
    -- 索引优化：用户类型、状态、名称索引
                                      INDEX `idx_user_type` (`UserType`),
                                      INDEX `idx_user_status` (`UserStatus`),
                                      INDEX `idx_user_name` (`UserName`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='统一用户表（包含管理员/普通用户，统一名称字段）';

-- 3. 竞赛主表（Competition）：无修改
...
-- 5. 团队-成员关联表（TeamMember）：无修改
CREATE TABLE `TeamMember` (
                              `TeamID` INT NOT NULL COMMENT '团队ID，关联Team表主键',
                              `userID` INT NOT NULL COMMENT '用户ID，关联User表主键',
                              `JoinTime` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '加入时间，默认当前时间',
...
CREATE TABLE IF NOT EXISTS `ChallengeHint` (
                                               ...
                                               `UpdateTime` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '提示更新时间（自动更新）',
                                               PRIMARY KEY (`HintID`),
...
-- 10. Flag提交记录表（FlagSubmission）：无修改
CREATE TABLE IF NOT EXISTS `FlagSubmission` (
                                                ...
                                                CONSTRAINT `fk_submission_competition` FOREIGN KEY (`CompetitionID`) REFERENCES `Competition` (`CompetitionID`) ON DELETE CASCADE,
                                                ...
                                                INDEX `idx_submission_competition` (`CompetitionID`),
...
-- 13. 分数汇总视图（ScoreSummary）：简化名称查询（直接取统一的UserName）
-- 注意：MySQL视图名称不区分大小写，但为了与实体类保持一致，使用ScoreSummary
CREATE OR REPLACE VIEW `Scoresummary` AS
WITH UserScoreStats AS (
    -- 统计用户个人分数和解题数据（直接取统一的UserName）
    SELECT
    ...
    GROUP BY u.userID, u.UserName, s.CompetitionID
),
     TeamScoreStats AS (
         -- 统计战队分数和解题数据（无修改）
         SELECT
...
-- 脚本执行完成提示
SELECT 'CTF竞赛系统数据库初始化完成（合并用户表+统一名称字段版）！' AS `Result`;